...
def gridTrade(stock,dlStock,atr,nowPrice,upPercent,downPercent,tradePercent,basePercent,sellAmt,cash):
    ...
    #获取买卖标志
    '''
    if nowPrice <= g.tradePrice  * (1-downPercent):
        g.buyFlag = 1
    elif nowPrice >= g.tradePrice * (1+upPercent) and g.tradePrice > 0:
        g.sellFlag = 1 
    elif g.buyFlag == 1 and g.sellFlag == 1:
        g.buyFlag = 0
    '''    
    ...
    if sellAmt == 0:
        ...
        #买入股票
        g.highPrice = nowPrice
        g.sarPrice = g.highPrice - 2 * atr
        order_value(stock, cash)
        log.info("buid base position,Buying %s,%f",stock,cash)
        #更新上一次的交易金额
        g.tradePrice = nowPrice
    else:
        if nowPrice >= g.tradePrice * (1+upPercent) and g.tradePrice > 0 and sellAmt > 0:
            #价格上涨，触发网格卖出交易
            #计算卖出的份额
            if sellAmt >= g.perTradeSellAmt:
                sellAmt = g.perTradeSellAmt 
            #卖出股票
            order_value(stock, -sellAmt)
            log.info("Selling %s,%f",stock,sellAmt)
            g.sellFlag = 0
            #更新上一次的交易金额
            g.tradePrice = nowPrice
        
        elif nowPrice <= g.tradePrice  * (1-downPercent):
            #价格下跌，触发网格买入交易
            #计算买入的份额
            #非建立底仓的情况下
            ...
            #买入股票
            order_value(stock, cash)
            log.info("Buying %s,%f",stock,cash)
            g.buyFlag = 0
            #更新上一次的交易金额
            g.tradePrice = nowPrice
        '''
    ...
    for stock in stocks:
        ...
        if lastClosePrice <= downCloudLine:
            ...
            if (g.lastMarket == 1 and dlStock > 0) or g.lastMarket == 0:
                ...
                gridTrade(stock,dlStock,ATR[-1],lastClosePrice,0.05,0.03,0.4,0.3,sellAmt,cash)
