...
def lowPEG_algo(context, lowPEG_ratio, portfolio_value):
    '''
    low PEG algorithms
    输入参数：lowPEG_ratio, protfolio_value（等同于total_value，已过时）
    ...
    #初始化了股票池，将上市不足60天的去掉，按照总风险为3%的设定将风险平均到5只股票
    g.lowPEG.fun_initialize(context)

    #当距离上次调仓过去30天或者手头上没有持有股票的时候调仓
    recal_flag = False
    ...
    if recal_flag:
        #按照99%的VAR置信度来进行风险平价
        #lowPEG_ratio值为1，代表这个策略的占总资产的仓位比重
...
class lowPEG_lib():
    ...
    def fun_assetAllocationSystem(self, context, buylist):
        def __fun_getEquity_ratio(context, __stocklist):
            __ratio = {}
            # 按风险平价配仓，这里是按照99%的的置信度，也就是防范最坏的1%的可能性
            if __stocklist:
    ...
    def fun_calPosition(self, context, equity_ratio, bonds_ratio, lowPEG_ratio, portfolio_value):
        ...
        #如果按照风险平价后的资金大于总资产，就不进行现金管理，反之则反
        #最终算出买股票的钱占到总的资产的的比例
        ...
        #按照之前通过99%置信度的ES算出的，和刚刚算出的资金比例，算出购买每个股票的金额
...
class quantlib():
    ...
    def fun_getEquity_value(self, equity_ratio, risk_money, maxrisk_money, confidence_ratio):
        ...
        if __portfolio_VaR:
            log.info("var is %f",__portfolio_VaR)
        ...
        __equity_value_ES = 0
        if __portfolio_ES:
            log.info("es is %f",__portfolio_ES)
        ...
        if equity_value == __equity_value_VaR:
            log.info("Use VAR to get Value")
        else:
            log.info("Use ES to get Value")
        return equity_value
    ...
    def fun_calStockWeight_by_risk(self, context, confidencelevel, stocklist):
        ...
        #ES是将历史上lag天的所有的收益数据按序排序，然后取收益率最差的lag*a日的平均风险值
        ...
        #这里主要是通过使用历史数据的收益率的方差来计算，而不是采用历史模拟，历史模拟是直接计算不超过a的概率中最差的情况
        ...
        #计算组合的ES，根据风险越大，所占比重越小的策略计算出各个资产组合的头寸
    ...
    #以昨日的价格为现价，计算当前的价值，如果头寸和现在的价值相差大于买卖的最小单位100股，则开始交易
    #在根据头寸和现值进行判断该买还是卖，同时对现金管理的银华日利进行特殊处理
    #即在卖银华日利的时候多卖100股
